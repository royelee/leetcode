...
ListNode* insertionSortList(ListNode* head)
{
    ...
    ListNode* itFromOldList = head;
    while( itFromOldList != nullptr )
    {
        ListNode* nextFromOldList = itFromOldList->next;
        int oldValue = itFromOldList->val;
        if( newHead == nullptr || newHead->val >= oldValue )
        {
            itFromOldList->next = newHead;
            newHead = itFromOldList;
            itFromOldList = nextFromOldList;
        }
        else
        {
            ListNode* itFromNewList = newHead;
            while( itFromNewList != nullptr )
            {
                ListNode* newListNext = itFromNewList->next;
                if( newListNext == nullptr ||
                   ( itFromNewList->val < oldValue && newListNext->val >= oldValue ) )
                {
                    itFromNewList->next = itFromOldList;
                    itFromOldList->next = newListNext;
                    itFromOldList = nextFromOldList;
                    break;
                }
                
                itFromNewList = newListNext;
            }
        }
    }
...
void testInsertionSortList()
{
    ...
    vector<int> v = ReadIntVectorFromFile("InsertionSortList");
