...
static vector<int> twoSum_binarysearch(vector<int>& nums, int target)
{
    ...
    for( int i = 0; i < nums.size(); i++ )
    {
        ...
        if( binary_search( sortedNums.begin(), sortedNums.end(), lookingFor ) )
        {
...
static vector<int> twoSum(vector<int>& nums, int target)
{
    ...
    {
        int lookingFor = target - nums[i];
        if( map.count( lookingFor ) ==  1 )
        {
            v.push_back( map[lookingFor] + 1);
            v.push_back( i + 1 );
...
ListNode* addTwoNumbers(ListNode* l1, ListNode* l2)
{
    ...
    ListNode* rit = result;  // result iterator
    ...
    while( it1 != nullptr )
    {
        if( it2 != nullptr )
        {
            int sum = it1->val + it2->val + addOne;
            ListNode* newNode = new ListNode(sum >= 10 ? sum - 10 : sum );
            if( sum >= 10 )
                addOne = 1;
            else
                addOne = 0;
...
ListNode* addTwoNumbers1(ListNode* l1, ListNode* l2)
{
    ...
    while( it1 != nullptr || it2 != nullptr )
    {
        ...
        else
        {
            ...
            int sum = (it1 != nullptr ? it1 : it2)->val + addOne;
            ListNode* newNode = new ListNode(sum >= 10 ? sum - 10 : sum );
            if( sum >= 10 )
                addOne = 1;
            else
    ...
    // This step is easy to miss. Notice it!!
    if( addOne == 1)
    {
        rit->next = new ListNode(1);
    }
...
//    ListNode* list2 = new ListNode(1);
    ...
    ListNode* r = addTwoNumbers1( list1, list2 );
    ListNode* it = r;
    while(it != nullptr)
    {
        cout << it->val << endl;
...
#pragma mark - lengthOfLongestSubstring

int continueMax(string s, int startIndex)
{
    int maxLength = 0;
...
bool isUniqueString(string s, int start, int length)
{
    bool unique = true;
    unordered_set<char> set;
    ...
    {
        if( set.find( substring[i]) == set.end() )
        {
            set.insert(substring[i]);
        }
        else
        {
            unique = false;
...
{
    int maxLength = 0;
    int lastRepeatCharIndex = 0;
    
    unordered_set<char> uniqueSubStrings;
    for( int i = 0; i < s.length(); i++ )
    {
        char c = s[i];
        if(uniqueSubStrings.find(c) == uniqueSubStrings.end())
        {
            // Not in the maps, then it's a unique to current substring.
            uniqueSubStrings.insert(c);
        }
        else
        {
            // Found repeat
            for( int j = lastRepeatCharIndex; j < i; j++ )
            {
                if( s[j] == s[i] )
                {
                    lastRepeatCharIndex = j + 1;
                    break;
                }

                uniqueSubStrings.erase(s[j]);
            }
        }
        
        maxLength = max( maxLength, (int)uniqueSubStrings.size() );
    }
...
string convert(string s, int numRows) {
    ...
    for( int i = 0; i < s.length(); i++ )
    {
        if( state == Down )
        {
            if( zigzag.size() == currentCol )
            {
                zigzag.push_back( vector<int>(numRows, -1) );
            }
            
            vector<int>& col = zigzag[currentCol];
            if( zigRow < numRows )
            {
                col[zigRow] = s[i];
                zigRow++;
    ...
    for( int i = 0; i < numRows; i++ )
    {
        ...
        {
            vector<int>& col = zigzag[j];

//             cout << (col[i] > 0 ? (char)col[i] : ' ') << "  ";
            if( col[i] != -1 )
                result.push_back((char)col[i]);
        }
...
void testZigzag()
{
    for( int i = -1; i < 5; i++)
    {
        string s = convert("PAYPALISHIRING", i);
        cout << "result = " << s << endl;
    }
}
